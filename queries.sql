...
WHERE ts>='2019-01-01 00:00:00' AND ts<='2019-06-30 23:59:59' AND id=anomalia_id 
GROUP BY email
HAVING COUNT(*)>=ALL (
    SELECT COUNT(*) 
    FROM anomalia NATURAL JOIN anomalia_traducao NATURAL JOIN incidencia NATURAL JOIN utilizador_regular 
...
WHERE email NOT IN (
    ...
    FROM (
        ...
        WHERE item_id=item.id AND anomalia_id=anomalia.id AND extract(year FROM ts)=2019 AND latitude>39.336775)
    ) AS R
);
